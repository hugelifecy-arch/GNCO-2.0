...
// ── Static constants (module scope — not inside component) ────
...
// ── Static params for all 15 jurisdictions ───────────────────
...
export default function JurisdictionDetailPage({
  ...
  const obligations = COMPLIANCE_OBLIGATIONS.filter(
    (entry) => entry.jurisdiction_id === jurisdiction.id,
  )
  const updates = REGULATORY_UPDATES.filter(
    (entry) =>
      entry.jurisdiction_id === jurisdiction.id && entry.status === 'active',
  )

  // Build vehicle rows from jurisdiction data
  const vehicleRows = jurisdiction.primaryVehicles.map((vehicle, index) => {
    ...
    const formation = Math.round(
      jurisdiction.formationCostRange.min +
        (jurisdiction.formationCostRange.max - jurisdiction.formationCostRange.min) *
          spreadFactor,
    )

    const annual = Math.round(
      jurisdiction.annualCostRange.min +
        (jurisdiction.annualCostRange.max - jurisdiction.annualCostRange.min) *
          spreadFactor,
    )

    const timelineMin = Math.max(
      1,
      Math.round(jurisdiction.setupTimeWeeks.min + spreadFactor),
    )
    const timelineMax = Math.max(
      timelineMin,
      Math.round(jurisdiction.setupTimeWeeks.max + spreadFactor),
    )
  ...
  // Build suitability scores for PE / VC / RE / Credit
  const suitability = STRATEGY_LABELS.map(({ strategy, label }) => ({
    ...
    score: Math.max(
      ...
      Math.min(
        10,
        Math.round(
          scoreJurisdiction(jurisdiction.id, {
            fundSize: 100,
            lpCount: 15,
            strategy,
          }) / 10,
    ...
    <main className="mx-auto max-w-7xl space-y-8 px-6 py-12">
      ...
      <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
        ...
        <KeyFact
          ...
          value={`${jurisdiction.setupTimeWeeks.min}–${jurisdiction.setupTimeWeeks.max} weeks`}
        />
        <KeyFact
          label="Headline tax"
          value={metadata?.tax_headline ?? 'See official tax authority guidance.'}
        />
        <KeyFact
          label="Min AUM"
          value={
            MIN_AUM_BY_JURISDICTION[jurisdiction.id] ??
            'No statutory minimum disclosed'
          }
        />
        <KeyFact
          label="Required providers"
          value={metadata?.service_providers ?? 'Administrator, auditor, legal counsel'}
        />
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <article className="rounded-xl border border-bg-border bg-bg-surface p-6">
          ...
          <p className="mt-3 text-sm text-text-secondary">
            {metadata?.tax_headline ?? jurisdiction.fatcaStatus}
          </p>
          ...
          <ul className="mt-2 space-y-1 text-sm text-text-secondary">
            {canonical ? (
              ...
              <li>WHT data not available for this jurisdiction.</li>
            )}
          </ul>

          <p className="mt-4 text-sm font-medium">Key treaty network</p>
          <p className="mt-2 text-sm text-text-secondary">
            {jurisdiction.taxTreaties.length
              ? jurisdiction.taxTreaties.join(', ')
              : 'Treaty data not available.'}
          </p>
        </article>
        ...
        <article className="rounded-xl border border-bg-border bg-bg-surface p-6">
          ...
          <ul className="mt-3 space-y-3 text-sm text-text-secondary">
            {obligations.length ? (
              ...
              <li>
                No jurisdiction-specific obligations currently in the structured feed.
              </li>
            )}
          </ul>
        </article>
      ...
      {/* Official links + Regulatory updates */}
      <section className="grid gap-4 lg:grid-cols-2">
        <article className="rounded-xl border border-bg-border bg-bg-surface p-6">
          ...
          <ul className="mt-3 space-y-2 text-sm">
            {metadata?.official_links?.regulator && (
              <li>
                ...
                <a
                  ...
                  href={metadata.official_links.regulator}
                  target="_blank"
                  rel="noreferrer"
                >
                  {metadata.official_links.regulator}
            ...
            {metadata?.official_links?.registry && (
              <li>
                ...
                <a
                  className="text-accent-gold"
                  href={metadata.official_links.registry}
                  target="_blank"
                  rel="noreferrer"
                >
                  {metadata.official_links.registry}
            ...
            {metadata?.official_links?.tax_authority && (
              <li>
                ...
                <a
                  className="text-accent-gold"
                  href={metadata.official_links.tax_authority}
                  target="_blank"
                  rel="noreferrer"
                >
                  {metadata.official_links.tax_authority}
            ...
            {!metadata?.official_links && (
              <li className="text-text-secondary">
                Official links not yet available for this jurisdiction.
              </li>
            )}
          </ul>
        </article>
      ...
      <section className="rounded-xl border border-bg-border bg-bg-surface p-6">
        ...
        <p className="mt-2 text-sm text-text-secondary">
          Last verified: {metadata?.last_verified_date ?? 'N/A'}
        </p>
