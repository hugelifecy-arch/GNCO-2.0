        ...
        <ResponsiveContainer>
          <PieChart>
            <Pie
              ...
              innerRadius={70}
              outerRadius={110}
              onClick={(_, idx) => setActiveFund(data[idx]?.name ?? null)}
            >
              {data.map((_, index) => (
                <Cell key={`slice-${index}`} fill={colors[index % colors.length]} opacity={activeFund ? (activeFund === data[index]?.name ? 1 : 0.45) : 1} />
              ))}
            </Pie>
            <Tooltip
              contentStyle={{ backgroundColor: '#0D1520', border: '1px solid #1E2D42', borderRadius: 8 }}
              formatter={(value: number, _n, p) => [`$${(value / 1_000_000).toFixed(0)}M`, `${p.payload.name}`]}
            />
          </PieChart>
        </ResponsiveContainer>
      </div>
        ...
        {data.map((fund, index) => (
          <button
            ...
            className={`flex items-center justify-between rounded border px-3 py-2 text-left ${
              activeFund === fund.name ? 'border-accent-gold/40 bg-accent-gold/10' : 'border-bg-border'
            }`}
